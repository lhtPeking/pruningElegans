...
class WiringRevised(nn.Module):
    ...
    def set_input_dim(self, input_dim):
        self.input_dim = input_dim
        self.sensory_adjacency_matrix = nn.Parameter(torch.zeros([input_dim, self.units], dtype=torch.float32))
    ...
    def add_synapse(self, src, dest, polarity):
        ...
        if not -1 <= polarity <= 1:
            raise ValueError(
                "Cannot add synapse with polarity {} (expected in range [-1, 1])".format(
                    polarity
                )
            )
    ...
    def add_sensory_synapse(self, src, dest, polarity):
        ...
        self.sensory_adjacency_matrix.data[src, dest] = polarity
...
class NCP(WiringRevised):
    ...
    def _build_sensory_to_inter_layer(self):
        ...
        self._pruning_and_rewiring(self.sensory_adjacency_matrix, self._num_sensory_neurons, self._inter_neurons)
    ...
    def _build_inter_to_command_layer(self):
        ...
        self._pruning_and_rewiring(self.adjacency_matrix, self._num_inter_neurons, self._command_neurons)
    ...
    def _build_recurrent_command_layer(self):
        ...
        self._pruning_and_rewiring(self.adjacency_matrix, self._num_command_neurons, self._command_neurons)
    ...
    def _build_command__to_motor_layer(self):
        ...
        self._pruning_and_rewiring(self.adjacency_matrix, self._num_command_neurons, self._motor_neurons)
